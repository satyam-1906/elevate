...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // WebGL not available – show a CSS gradient fallback instead of crashing
    ...
    const handlePointerMove = event => {
      // Use canvas bounding rect to convert global cursor → canvas-local coords
      // This works even when the mouse is over HTML content layered above the canvas
      const rect = renderer.domElement.getBoundingClientRect();
      const x = event.clientX - rect.left;
      const y = event.clientY - rect.top;
      const dpr = renderer.getPixelRatio();

      // Only update if cursor is inside (or near) the canvas region
    ...
    // Detect when cursor leaves the canvas region using document-level check
    const handlePointerLeave = event => {
      ...
      if (outside) targetInfluenceRef.current = 0.0;
    };

    if (interactive) {
      // Listen on window so events reach us through any overlapping DOM elements
      window.addEventListener('pointermove', handlePointerMove, { passive: true });
      window.addEventListener('pointermove', handlePointerLeave, { passive: true });
    }
